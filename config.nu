...
# -------- Aqui começa o que eu mudei ----------------

source ~/.cache/starship/init.nu

def-env tp [place = "none"] {
  let $favorites = [
    ["lyah" d:\Projects\learn-you-a-haskell\] 
    ["crab" d:\Projects\rust\]
    ["stp" d:\bolsa\projeto_stop_logs\]
    ["util" c:\utilitarios\]
    ["esp" d:\espstuff\8266\msys32\home\guilh\esp8266\]
  ]

  if $place == "none" {
    echo "Usage: tp place"
    $favorites | each { |fav| echo $"($fav.0) => ($fav.1)" }
  } else {
    let $matches = ($favorites | each { |fav| if $fav.0 == $place {echo $fav.1}})
    if ($matches | length) < 1 { 
      echo "Unknown place, run without arguments to discover them."
    } else { 
      cd $matches.0 
    }
  }
